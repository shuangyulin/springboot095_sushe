<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="丢失时间" prop="diushishijian">
            <el-date-picker
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.diushishijian" label="丢失时间" prop="diushishijian">
              <el-input v-model="ruleForm.diushishijian" 
                placeholder="丢失时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="寻物状态" prop="xunwuzhuangtai">
          <el-select v-model="ruleForm.xunwuzhuangtai" placeholder="请选择寻物状态">
            <el-option
                v-for="(item,index) in xunwuzhuangtaiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="寻物状态" prop="xunwuzhuangtai">
	      <el-input v-model="ruleForm.xunwuzhuangtai"
                placeholder="寻物状态" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="详细描述" prop="xiangximiaoshu">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.xiangximiaoshu" label="详细描述" prop="xiangximiaoshu">
                    <span v-html="ruleForm.xiangximiaoshu"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	shouji : false,
      ...
      rules: {
          ...
          wupinleibie: [
                { required: true, message: '物品类别不能为空', trigger: 'blur' },
          ],
          diushididian: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.diushishijian = true;
          ...
          if(o=='xunwuzhuangtai'){
            this.ruleForm.xunwuzhuangtai = obj[o];
	    this.ro.xunwuzhuangtai = true;
            continue;
          }
          if(o=='xiangximiaoshu'){
      ...
      this.$http({
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/wupinleibie/wupinleibie`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
            ...
            this.xunwuzhuangtaiOptions = "未寻回,已寻回".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `shiwuxinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.xiangximiaoshu = this.ruleForm.xiangximiaoshu.replace(reg,'../../../springboot68ozj/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shiwuxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shiwuxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
