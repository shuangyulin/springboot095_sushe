<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="检查内容" prop="jianchaneirong">
          <el-input v-model="ruleForm.jianchaneirong" 
              placeholder="检查内容" clearable  :readonly="ro.jianchaneirong"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="检查内容" prop="jianchaneirong">
              <el-input v-model="ruleForm.jianchaneirong" 
                placeholder="检查内容" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="是否整洁" prop="shifouzhengjie">
          <el-select v-model="ruleForm.shifouzhengjie" placeholder="请选择是否整洁">
            <el-option
                v-for="(item,index) in shifouzhengjieOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="是否整洁" prop="shifouzhengjie">
	      <el-input v-model="ruleForm.shifouzhengjie"
                placeholder="是否整洁" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="是否违规" prop="shifouweigui">
          <el-select v-model="ruleForm.shifouweigui" placeholder="请选择是否违规">
            <el-option
                v-for="(item,index) in shifouweiguiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="是否违规" prop="shifouweigui">
	      <el-input v-model="ruleForm.shifouweigui"
                placeholder="是否违规" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="卫生评分" prop="weishengpingfen">
          <el-input v-model="ruleForm.weishengpingfen" 
              placeholder="卫生评分" clearable  :readonly="ro.weishengpingfen"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="卫生评分" prop="weishengpingfen">
              <el-input v-model="ruleForm.weishengpingfen" 
                placeholder="卫生评分" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="检查结果" prop="jianchajieguo">
          <el-select v-model="ruleForm.jianchajieguo" placeholder="请选择检查结果">
            <el-option
...
export default {
  data() {
	...
	banji : false,
	sushehao : false,
	jianchaneirong : false,
	shifouzhengjie : false,
	shifouweigui : false,
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.sushehao = true;
          ...
          if(o=='jianchaneirong'){
            this.ruleForm.jianchaneirong = obj[o];
	    this.ro.jianchaneirong = true;
            continue;
          }
          if(o=='shifouzhengjie'){
            this.ruleForm.shifouzhengjie = obj[o];
	    this.ro.shifouzhengjie = true;
            continue;
          }
          if(o=='shifouweigui'){
            this.ruleForm.shifouweigui = obj[o];
	    this.ro.shifouweigui = true;
            continue;
          }
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.shifouzhengjieOptions = "是,否".split(',')
            this.shifouweiguiOptions = "是,否".split(',')
            this.jianchajieguoOptions = "合格,不合格".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `weishengjiancha/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `weishengjiancha/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.weishengjianchaCrossAddOrUpdateFlag = false;
                  this.parent.search();
